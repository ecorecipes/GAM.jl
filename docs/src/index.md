...
- **30 registered smooth basis types**: thin plate (`:tp`, `:ts`), cubic (`:cr`, `:cs`, `:cc`),
  ...
  One (`:so`) is a documented approximation of its mgcv namesake; `:ds` is an
  alias of `:tp` that warns rather than posing as a Duchon spline

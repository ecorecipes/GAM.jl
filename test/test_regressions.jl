...
@testset "Targeted regressions" begin
    ...
    @testset "Tensor smooth2random follows the mgcv convention" begin
        n = 120
        x = randn(rng, n)
        z = randn(rng, n)
        df = DataFrame(x = x, z = z, y = x .* z .+ 0.1 .* randn(rng, n))

        # t2 is *designed* to decompose into independent random-effect blocks
        # (that is its purpose in lme4/gamm4): the Wood, Scheipl & Faraway
        ...
        @test sum(group_counts) == length(smm2.rind)
        # penalties really are diagonal with disjoint support
        supports = [Set(findall(j -> maximum(abs, S[:, j]) > 1e-10, axes(S, 2)))
                    for S in sm2.S]
        @test all(maximum(abs, S - Diagonal(diag(S))) < 1e-10 for S in sm2.S)
        @test all(isempty(intersect(supports[a], supports[b]))
                  for a in 1:(length(supports) - 1) for b in (a + 1):length(supports))

        # te penalties overlap and CANNOT be written as independent i.i.d.
        # blocks — mgcv needs the pdTens class in gamm, and
        ...
        @test count(==(1), smmte.pen_ind) == length(smmte.rind)

        # both paths reassemble exactly: β_original = U * (D .* [b; β_f])
        for (sm_i, smm_i) in ((sm2, smm2), (smte, smmte))
            c = randn(rng, sum(size(Z, 2) for Z in smm_i.Zs) + size(smm_i.Xf, 2))
            β = smm_i.trans_U * (smm_i.trans_D .* c)
            @test sm_i.X * β ≈ hcat(smm_i.Zs..., smm_i.Xf) * c atol = 1e-10
        end
    end

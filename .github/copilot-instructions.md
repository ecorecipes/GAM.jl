...
| variable | effect |
|---|---|
| `GAM_RCALL_ONLY=true` | only the 17 R-comparison files (~812 assertions, ~6 min) |
| `GAM_SKIP_RCALL=true` | skip everything needing R (~9081 assertions) |
...
**Do not "simplify" the R availability probe in `test/runtests.jl`.** It runs
`using RCall` in a *subprocess* on purpose: loading RCall maps libR into the
...
CI's R job currently skips the comparisons because RCall cannot load R on that
runner image. That is reported, not hidden. The comparisons do run locally
where R works.
...
Five gating jobs — Julia 1.11/ubuntu, Julia 1/ubuntu, macOS, Windows, and the
docs build — plus an informative R-comparison job marked `continue-on-error`.

...
Note that $f_3(x_3) = 0$ (the null smooth), and the model should estimate it as approximately flat with a small EDF (near 1 — without `select=true` a smooth's EDF cannot shrink below its unpenalized null space, so exact removal requires the double-penalty approach of `gam(...; select=true)`).

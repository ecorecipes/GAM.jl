...
In GAM.jl, `gamm(@gamm_formula(y ~ s(x) + (1|subject)), ...)` is mathematically equivalent to `gam(@formulak(y ~ s(x) + s(subject, bs=:re)), ...)`. Both treat the random intercept as a smooth with identity penalty:

...
Note when comparing with the Julia side: the *common* smooth's edf agrees
closely between the two packages, but the deviation term's does not, and the
two implementations reach a similar deviance by different splits of the
effective degrees of freedom. Compare deviance and the fitted curves here
rather than the per-term edf.

...
```
    ...
    Precompiling packages...
      53256.0 ms  ✓ GAM
      1 dependency successfully precompiled in 76 seconds. 118 already precompiled.
    Precompiling packages...
      29037.9 ms  ✓ GAM
    Info Given GAMPlotsExt was explicitly requested, output will be shown live 
    ┌ Warning: Module GAM with build ID fafbfcfd-ec14-04eb-fdd7-982370dff7da is missing from the cache.
    │ This may mean GAM [f5b95e24-6450-466f-92b1-fee0c65c0d6b] does not support precompilation but is imported by a module that does.
    └ @ Base loading.jl:2641
        558.7 ms  ? GAM → GAMPlotsExt
      1 dependency successfully precompiled in 121 seconds. 238 already precompiled.
...
The dataset is a checked-in CSV following exactly this data-generating
process; unlike most vignette data it is *not* regenerated by
`vignettes/generate_data.jl` (see that script’s header), so treat the
file itself as the source of record.

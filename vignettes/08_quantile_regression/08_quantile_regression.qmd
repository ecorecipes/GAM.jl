...
`mqgam` is a convenience wrapper that loops over the requested quantiles,
fitting an independent `qgam` (with its own calibration) for each and
collecting the results in one object:
